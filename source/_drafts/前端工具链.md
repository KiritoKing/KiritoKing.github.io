...
现在我们使用的npm版本是**v8**，其node_modules目录也是熟悉的扁平化目录结构，那它究竟是如何一步步发展到这一步的呢？

- npm2会**嵌套地**安装每一个包所依赖的所有依赖项，这可能产生很深的依赖路径，就像下面这样。这对Unix来说可能无伤大雅，但对于Windows来说路径过长就是一场灾难（因为有很多程序无法处理超过260个字符的文件路径名）

  <img src="./前端工具链.assets/4386dbd291331a140b8353dcec8b961fe99300-1683191839838-7.jpg" alt="img" style="zoom: 50%;" />

- npm3采用了**扁平依赖关系树**来解决上面的问题，因此变成了下面这样。但这种方法有一些缺点：

  ![img](./前端工具链.assets/213ed4b459fed61d4d2236af5c9e10423628da-1683191864453-11.png)

  - npm必须首先遍历所有的项目依赖关系，然后再决定如何生成扁平的node_modules目录结构（即在install安装时，会不停的往上级node_modules中寻找，如果找到同样的包，就不再重复安装）。npm必须为所有使用到的模块构建一个完整的依赖关系树，这是一个耗时的操作，是[npm安装速度慢的一个很重要的原因](https://github.com/npm/npm/issues/8826)。
  - 依赖结构是不确定的（特别是在不同版本依赖上），且项目中仍然可以非法访问没有声明过依赖的包（这个问题被后来的**lock-file**解决了，是[yarn](#Yarn)率先解决了此问题）
...
cnpm是一个中国镜像源（切换为淘宝源）的npm，对npm的机制并没有做出改变，这里不做推荐。

如果真的需要镜像源管理，推荐使用[nrm]()（如果只有镜像源管理需求），或如果你正好有在使用[nvm]()的话也可以直接使用nvm的镜像源管理功能。

### pnpm

> *pnpm*(performant npm) is a **fast, disk space efficient package manager**

上述是[pnpm官网](https://pnpm.io/)对自己的定位和介绍，可以看出其是一款对标npm和yarn的包管理，主打一个**快速和省空间**，不仅拥有npm&yarn的全部特性（lock-file固定化依赖、全局下载缓存、yarn的并行计算等），还拥有一些独特的优势，如**硬链接共享依赖、支持mono-repo**等。
...
![img](./前端工具链.assets/f1a984784cc4f415464644f0e48003526b662f-1683191966615-15.png)
